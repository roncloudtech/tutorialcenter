  ...
  return (
    <>
      <div
        ...
        } h-screen grid p-3 transition-all ease-custom overflow-y-hidden scroll`}
      >
        {/* LEFT */}
...
const SideBar = ({ toggle, setToggle }) => {
  return (
    <div
      className={`scroll position bg-[#D1D5DB] p-3 flex flex-col gap-4 rounded-lg relative ${
        toggle ? "" : "items-center "
      }`}
    >
      ...
      <div className="flex items-center gap-2">
        <Icon
          ...
          style={{ color: "#000" }}
        />
        <span className={`${toggle ? "block" : "hidden"} text-xs font-medium`}>
          Logout
        </span>
      </div>
    </div>
